...
### 数据库迁移紧跟上游最大号，**不要跳到高位号段**

先说结论：加迁移前 `ls migrations/versioned/*.up.sql | tail -3` 看上游到哪了，
接着往下排。**不要为了避让而跳到 000900 这种高位号。**

这条规矩改过一次，两次都是踩出来的：

**第一次（2026-08-20）**：我们和上游同时用了 `000080`。golang-migrate 以版本号
为键，同号两份只认一个，**另一个被静默跳过**，`schema_migrations` 却显示一切正常。
当时的结论是"跳到 000900 去，上游短期到不了那里"。

**第二次（2026-08-27）**：这个结论是错的，而且错得更隐蔽。迁移按号从小到大跑，
库停在 900 之后，**上游新增的 85~88 因为号更小，永远轮不到**。表现是：

    上游新增 000085~000088（技能表等四个）
    库里版本 = 900
    → 四个迁移一个都没执行，而且没有任何报错

避开了撞号，代价是**从此上游的所有新迁移都不会执行**。这比撞号更难发现——
撞号至少还有一半会跑，跳号是全部静默失效。

修复办法（那次做过一遍）：把我们的迁移重编号插回上游后面，再把
`schema_migrations` 手动退回到上游最大号，让后续按顺序跑一遍。动手前先备份
`select version, dirty from schema_migrations` 的值。

所以：

- **紧跟上游最大号往下排**，撞号靠加之前先看一眼来避免，不靠跳号段
- 迁移里一律用 `IF NOT EXISTS` / `IF EXISTS`，让重复应用是安全的——
  重编号时要把库退回去重跑，不幂等就会炸
- 合并上游后确认一次 `schema_migrations` 的版本号有没有前进
