...
const MARKDOWN_SAMPLE = `# TeKnowra 知识框架

TeKnowra 是一个基于 LLM 的企业知识框架，集 RAG 问答、ReAct 智能体、Wiki 知识图谱于一体。本文介绍其设计动机、架构与典型用法。

## 设计动机

企业内部知识散落在 Confluence、飞书、Notion、Git 仓库等多种系统，传统全文检索难以理解语义，单一 LLM 又缺少可信赖的上下文来源。TeKnowra 的目标是：
...
\`\`\`bash
git clone https://git.example.com/teknowra.git && cd teknowra
...
const FAQ_SAMPLE = `# TeKnowra 部署与使用 FAQ
...
\`\`\`
docker pull registry.example.com/teknowra/app:v1.0.0
docker pull registry.example.com/teknowra/docreader:v1.0.0
docker pull registry.example.com/teknowra/ui:v1.0.0
\`\`\`
