...
import { installSkillCatalog, type SkillCatalogItem, type SkillInfo } from '@/api/skill';
import {
...
// requires_tools 的数据源：目录接口(SkillCatalogItem)不带 requires_tools，
// 缺工具校验仍从 GET /api/v1/skills 取（那份带 requires_tools），两份并存。
...
async function syncInstalledSkills(force = false) {
  ...
  // requires_tools 从这份按沙箱列的技能清单来；目录接口不带它。
