export default {
  localBrowser: {
    ...
    "copyPairing": "复制配对链接",
    "copied": "已复制",
    "windowHint": "任务在 Chrome 的 TeKnowra 标签组内运行。借用已有标签页需确认。",
    "preview": "本机浏览器任务预览",
    "waiting": "等待任务页面",
  ...
  integrations: {
    ...
    claw: {
      title: 'WeKnora Skill',
      subtitle: '通过 TeKnowra REST API 导入文档并执行混合检索（向量 + 关键词）。适用于上传文件/URL/Markdown 到知识库、跨库检索与浏览知识内容。',
    ...
    chrome: {
      title: '知识管理助手',
      subtitle: '配合 TeKnowra 自建服务使用：在任意网页侧边栏提问、剪藏内容、Markdown 速记，将浏览中的知识沉淀到你的知识库。',
      ...
      steps: {
        ...
        port: {
          title: '桌面版配置固定端口（推荐）',
          desc: '使用 TeKnowra 桌面版时，在 API 信息中设置固定端口（如 37841），避免每次启动后地址变化导致插件断连。'
        },
        api: {
          title: '获取 API 凭证',
          desc: '在「设置 → API 信息」中复制 API Key 与 API 地址。'
    ...
    api: {
      ...
      signedFlowDetail: 'HMAC 密钥仅保存在 TeKnowra 与你的可信后端，切勿放入请求或下发给浏览器。请求头里传的是用该密钥签出的 JWT 字符串（每次签发或过期后会变），不是密钥本身。JWT 须包含 sub（外部用户 ID）、tenant_id、aud=teknowra、exp（有效期不超过 24 小时）。',
  ...
  agentEditor: {
    ...
    tools: {
      ...
      mcpLoading: '正在读取该服务的工具…',
      mcpLoadFailed: '取不到该服务的工具：{message}。请到「MCP 服务」检查地址与凭据。',
      dangerTag: '可写',
  ...
  envVarSettings: {
    title: '沙箱密钥',
    description: '给技能和沙箱用的个人密钥，不是 TeKnowra 的系统或部署配置。',
    helpAria: '沙箱密钥说明',
  ...
  settings: {
    ...
    sandbox: {
      ...
      dockerHostRisk: '留空或 unix:// 会使用 TeKnowra 所在机器的 Docker 守护进程，权限等同该机 root，只适合私有化单机。多套空间共用同一主机时请改用 Cube 或 E2B。远程 tcp:// 必须填写 TLS 证书目录。',
      addConfig: '添加沙箱',
      ...
      weknoraTemplateTitle: 'TeKnowra 标准模板',
      weknoraDockerImage: 'TeKnowra 标准镜像',
      weknoraDockerImageHint: '每次会话独占一个长驻容器，脚本、shell 命令与文件都在同一个容器内，会话结束或空闲超时后回收。',
      weknoraTemplateOverview: 'TeKnowra 负责提供标准运行环境。连接集群后会自动发现模板，缺少标准模板时自动创建。',
      weknoraTemplateDescription: '包含 TeKnowra 技能所需的 Python、Node.js、CLI 工具、工作目录和非 root 执行用户。',
      recommendedTag: '推荐',
      ...
      templateLoadHint: '填写集群连接后刷新模板。没有 TeKnowra 标准模板时点「创建」；改 DNS 或镜像后点「删除并重建」。',
      templateLoadFailed: '模板列表加载失败',
      standardTemplateProvisioning: '已开始在集群中创建 TeKnowra 标准模板，请稍后刷新查看状态',
      ...
      templateStepHint: '进入此步只会列出集群已有模板，不会自动创建。没有标准模板时点「创建 TeKnowra 标准模板」；改 DNS 或镜像后点「删除并重建」。模板就绪后才能继续。',
      loadingTemplates: '正在从集群加载模板…',
      templateBuildingHint: '标准模板正在构建，列表会自动刷新。',
      templateUntaggedHint: '构建已完成，但没有构建带 default 标签，创建沙箱时无法解析。请在 E2B 删除该模板，刷新后 TeKnowra 会重新构建。',
      templateFailedReason: '构建失败：{reason}',
      noTemplates: '当前集群未返回可用模板。',
      weknoraStandardTemplate: 'TeKnowra 标准模板',
      createStandardTemplate: '创建',
      createStandardTemplateHint: '按当前连接配置构建，包含 DNS。之后改配置可在卡片上重建。',
      replaceStandardTemplate: '重建',
      replaceStandardTemplateConfirm: '将用当前配置（含 DNS）重建 TeKnowra 标准模板。新模板就绪前不会删除仍可启动的旧模板。',
      ...
      cubeDnsServersHelp: '可选。写入 TeKnowra 标准模板的 nameserver（须为 IP）。留空则使用集群默认（常见 119.29.29.29）。私网或云上 UDP 53 出不去时，填 Cube 宿主机 /etc/resolv.conf 里能用的地址，并避开 10/8、172.16/12、192.168/16。已有标准模板需在模板卡片上点「重建」才会生效。',
      ...
      dockerTlsCertPathHelp: 'TeKnowra 所在主机上包含 ca.pem、cert.pem、key.pem 的目录。远程守护进程必填，证书不入库，由部署方挂载。',
      dockerIdleTtl: '空闲回收（秒）',
      dockerIdleTtlHelp: 'Docker 守护进程本身没有空闲超时。容器多久没有执行任何命令就会被 TeKnowra 回收，会话继续时重建。留空按 1800 秒。',
