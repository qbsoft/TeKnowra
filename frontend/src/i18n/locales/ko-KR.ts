export default {
  ...
  integrations: {
    ...
    claw: {
      title: 'WeKnora Skill',
      subtitle: 'TeKnowra REST API로 문서를 가져오고 하이브리드 검색(벡터+키워드)을 수행합니다. 파일/URL/Markdown 업로드 및 검색에 사용.',
    ...
    chrome: {
      title: '지식 관리 어시스턴트',
      subtitle: '자체 호스팅 TeKnowra와 함께: 사이드바에서 질문하고, 웹 페이지를 클리핑하며, Markdown 메모를 지식베이스에 저장하세요.',
      ...
      steps: {
        ...
        port: {
          title: '데스크톱: 고정 포트(권장)',
          desc: 'TeKnowra 데스크톱에서는 API 정보에서 고정 포트(예: 37841)를 설정하세요.'
        },
        api: {
          title: 'API 자격 증명',
    ...
    api: {
      ...
      signedFlowDetail: 'HMAC secret은 TeKnowra와 신뢰할 수 있는 백엔드에만 보관하세요. 요청에 넣거나 브라우저에 배포하지 마세요. 요청 헤더에는 secret이 아니라 해당 secret으로 서명한 JWT 문자열을 넣습니다(발급마다 또는 만료 후 변경). JWT에는 sub(외부 사용자 ID), tenant_id, aud=teknowra, exp(최대 24시간)가 필요합니다.',
      ...
      hmacSecretDesc: 'TeKnowra에 저장된 것과 동일한 secret. 백엔드에서 JWT 서명에만 사용하며 요청 헤더로 보내지 않습니다.',
      ...
      signedRequestStep0: '# 0. 백엔드에서 JWT 서명 (HS256, aud=teknowra, sub=사용자 ID, tenant_id={tenantId}, exp 최대 24시간)',
  ...
  agentEditor: {
    ...
    tools: {
      ...
      mcpLoading: '이 서비스의 도구를 불러오는 중…',
      mcpLoadFailed: '이 서비스의 도구를 가져오지 못했습니다: {message}. MCP 서비스에서 주소와 자격 증명을 확인하세요.',
      dangerTag: '쓰기',
  ...
  createChat: {
    title: '안녕하세요, TeKnowra입니다 — 당신의 지식을 손끝에',
  ...
  auth: {
    ...
    registerSubtitle: '계정을 만들고 TeKnowra를 시작하세요',
  ...
  envVarSettings: {
    title: '샌드박스 키',
    description: '스킬과 샌드박스에 쓰는 개인 키이며, TeKnowra 시스템이나 배포 설정이 아닙니다.',
    helpAria: '샌드박스 키 설명',
  ...
  settings: {
    ...
    sandbox: {
      ...
      backendDescriptions: {
        ...
        docker: '이 TeKnowra 호스트의 Docker에서 세션마다 장수명 컨테이너를 유지합니다. 스크립트와 파일이 같은 컨테이너에 남습니다',
      },
      ...
      dockerHostRisk: '비우거나 unix:// 를 쓰면 TeKnowra가 있는 머신의 Docker 데몬을 사용하며, 그 권한은 해당 머신의 root와 같습니다. 프라이빗 단일 노드에만 쓰세요. 여러 워크스페이스가 같은 호스트를 쓰는 경우에는 Cube 또는 E2B를 쓰세요. 원격 tcp:// 는 TLS 인증서 디렉터리가 필요합니다.',
      addConfig: '샌드박스 추가',
      ...
      templateLoadHint: '클러스터 연결을 입력한 뒤 새로고침하세요. TeKnowra 표준 템플릿이 없으면 「만들기」를, DNS나 이미지를 바꾼 뒤에는 「삭제 후 재구축」을 누르세요.',
      ...
      templateStepHint: '이 단계에서는 클러스터에 이미 있는 템플릿만 나열하며 자동으로 만들지 않습니다. 표준 템플릿이 없으면 「TeKnowra 표준 템플릿 만들기」를, DNS나 이미지를 바꾼 뒤에는 「삭제 후 재구축」을 누르세요. 템플릿이 준비된 뒤에 계속할 수 있습니다.',
      loadingTemplates: 'Loading templates from the cluster...',
      templateBuildingHint: '표준 템플릿을 구축하는 중입니다. 목록이 자동으로 새로고침됩니다.',
      templateUntaggedHint: '빌드는 끝났지만 default 태그가 붙은 빌드가 없어 샌드박스 생성 시 이 템플릿을 찾을 수 없습니다. E2B에서 템플릿을 삭제하고 새로고침하면 TeKnowra가 다시 빌드합니다.',
      templateFailedReason: '빌드 실패: {reason}',
      noTemplates: 'No templates were returned by this cluster.',
      weknoraStandardTemplate: 'TeKnowra 표준 템플릿',
      ...
      replaceStandardTemplateConfirm: '현재 설정(DNS 포함)으로 TeKnowra 표준 템플릿을 다시 만듭니다. 새 템플릿이 준비되기 전에는 기존에 쓸 수 있는 템플릿을 삭제하지 않습니다.',
      ...
      cubeDnsServersHelp: '선택 사항입니다. TeKnowra 표준 템플릿에 기록되는 nameserver IP입니다. 비워 두면 클러스터 기본값(보통 119.29.29.29)을 사용합니다. 공용 DNS로 UDP/53이 나가지 않으면 Cube 호스트 /etc/resolv.conf에서 도달 가능한 주소를 넣고, 10/8, 172.16/12, 192.168/16은 피하세요. 이미 있는 표준 템플릿은 카드에서 「재구축」을 눌러야 변경이 적용됩니다.',
  ...
  newUserGuide: {
    ...
    steps: {
      ...
      knowledge: {
        title: '지식 베이스 만들기',
        desc: '모든 것의 시작점입니다. 문서, 웹페이지, FAQ를 업로드하면 TeKnowra가 자동으로 분석하고 색인합니다. 여기를 클릭해 지식 베이스로 이동하세요.'
      },
      welcome: {
        title: 'TeKnowra에 오신 것을 환영합니다',
