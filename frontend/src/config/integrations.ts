 ...
 * Chrome 插件同理：那是上游发布在 Chrome 商店的「知识管理助手」，源码不在本仓库，
 * 弹窗里的「WeKnora 登录」、your-weknora-domain 等字样我们改不了。
 * （与「浏览器连接」页的 BrowserSkill 扩展无关，那个是随 app 镜像分发的配套构建。）
 * INTEGRATION_TABS 不动：路由仍可解析，页面仍可用 ?section= 直接打开。见 BRANDING.md。
 */
const HIDDEN_INTEGRATION_TABS: ReadonlySet<IntegrationTab> = new Set(['cli', 'chrome', 'claw'])
